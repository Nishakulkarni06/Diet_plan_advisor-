...
// /* ---------- Ayurvedic Formatter ---------- */
// const formatAyurvedic = (item) => {
//   if (!item) return "-";
//   const rasa = item.Ayurvedic_Rasa ? `${item.Ayurvedic_Rasa}` : "";
//   const guna = item.Ayurvedic_Guna ? `${item.Ayurvedic_Guna}` : "";
//   const virya = item.Ayurvedic_Virya ? `${item.Ayurvedic_Virya}` : "";

//   const sentenceParts = [];
//   if (rasa) sentenceParts.push(`The food mainly tastes ${rasa} (${rasa === "Sweet" ? "Madhura Rasa" : rasa}).`);
//   if (guna) sentenceParts.push(`It is ${guna.toLowerCase()} to digest (${guna === "Easy" ? "Laghu Guna" : guna}).`);
//   if (virya) sentenceParts.push(`It has a ${virya.toLowerCase()} effect (${virya === "Cold" ? "Shita Virya" : virya}).`);
...
//           `${formatValue(it.Carbohydrates_g)} g`,
//           `${formatValue(it.Fat_g)} g`,
//           formatAyurvedic(it), // <-- Use the formatted Ayurvedic sentence here
//           `${it.Notes || ""}`,
//         ];
...
//                         <th className="px-4 py-2 border-b text-left w-1/12">Carbs</th>
//                         <th className="px-4 py-2 border-b text-left w-1/12">Fat</th>
//                         <th className="px-4 py-2 border-b text-left w-2/12">Ayurvedic Properties</th>
//                         <th className="px-4 py-2 border-b text-left w-2/12">Notes / Purpose</th>
//                       </tr>
//                     </thead>
//                     <tbody>
...
//                             <EditableMealCell value={`${formatValue(item.Adjusted_Calories_kcal)} kcal`} isEditing={false} />
//                             <EditableMealCell value={`${formatValue(item.Protein_g)} g`} isEditing={false} />
//                             <EditableMealCell value={`${formatValue(item.Carbohydrates_g)} g`} isEditing={false} />
//                             <EditableMealCell value={`${formatValue(item.Fat_g)} g`} isEditing={false} />

//                             <EditableMealCell
//                               value={formatAyurvedic(item)}
//                               isEditing={editingCell.dayIdx === dayIdx && editingCell.itemIdx === itemIdx && editingCell.field === "Ayurvedic"}
//                               editValue={editValue}
//                               setEditValue={setEditValue}
//                               startEdit={() => startEdit(dayIdx, mealType, itemIdx, "Ayurvedic", `${item.Ayurvedic_Rasa} / ${item.Ayurvedic_Guna} / ${item.Ayurvedic_Virya}`)}
...
export default function DietPlanPage() {
 ...
 const generatePDF = () => {
  ...
  const pageWidth = doc.internal.pageSize.width;
  const usableWidth = pageWidth - margin * 2;
  const colWidths = [120, 50, 45, 45, 45, 100, 100]; // Food & Portion + 6 other cols
  const lineHeight = 12;
  let y = 60;
  ...
  plan.recommendedMeals.forEach((day) => {
    ...
    ["breakfast", "lunch", "dinner"].forEach((mealType) => {
      const items = day.meals[mealType]?.items || [];
      ...
      doc.rect(margin, y, usableWidth, mealHeaderHeight, "F");
      doc.text(mealType.charAt(0).toUpperCase() + mealType.slice(1), margin + 5, y + 13);
      y += mealHeaderHeight + 5;

      // Table headers
      doc.setFontSize(10);
      doc.setTextColor("#000");
      const headers = ["Food & Portion", "Calories", "Protein", "Carbs", "Fat", "Ayurvedic Properties", "Notes / Purpose"];
      let x = margin;
      const headerHeight = lineHeight + 4;
      headers.forEach((h, i) => {
        const w = colWidths[i];
        doc.rect(x, y, w, headerHeight);
        const splitHeader = doc.splitTextToSize(h, w - 4);
        doc.text(splitHeader, x + 2, y + 12);
        x += w;
      });
      y += headerHeight;
      ...
      items.forEach((it) => {
        ...
        // Calculate max cell height dynamically
        const cellHeights = row.map((cell, i) => doc.splitTextToSize(cell, colWidths[i] - 4).length * lineHeight + 4);
        const rowHeight = Math.max(...cellHeights);
        ...
        row.forEach((cell, i) => {
          ...
          const splitText = doc.splitTextToSize(cell, w - 4);
          doc.text(splitText, x + 2, y + 12);
          x += w;
        });
        y += rowHeight;
      ...
      y += 10; // spacing after meal box
    });

    y += 10; // spacing after day
  });
              ...
              <div key={dayIdx} className="mb-8">
                ...
                <div className="space-y-4">
                  {["breakfast", "lunch", "dinner"].map((mealType) => {
                    ...
                    return (
                      <div key={mealType} className="bg-white rounded-2xl shadow border border-slate-200 p-4">
                        <h3 className="font-bold text-emerald-700 mb-2 text-lg">{mealType.charAt(0).toUpperCase() + mealType.slice(1)}</h3>
                        <div className="overflow-x-auto">
